...
├── IMessageHistory.cs      # Интерфейс для работы с историей
├── IMessageProcessor.cs    # Интерфейс для обработки сообщений
├── LoginWindow.xaml        # Окно ввода имени
├── LoginWindow.xaml.cs     # Логика авторизации (валидация без MessageBox)
├── MainWindow.xaml         # Главное окно чата
...
|----|------------------------------------------------------------------------|-----------|----------------------------------------------------------------------------|
| 1  | Бот принимает строки вида "231 - 123" (прямые арифметические выражения) | ✅ READY  | Поддерживаются +, -, *, / с учётом отрицательных чисел и пробелов          |
| 2  | Убрано диалоговое окно при ошибке ввода имени                          | ✅ READY  | Ошибка отображается красным текстом под полем ввода (современный UX)      |
| 3  | Сделаны два интерфейса: для истории и для обработки сообщений          | ✅ READY  | IMessageHistory и IMessageProcessor — разделение ответственности по SOLID |
| 4  | Добавлена UML-диаграмма классов                                        | ✅ READY  | Чистая, минималистичная, с тремя типами связей (реализация, зависимость, композиция) |
| 5  | Исправлена статистика чата (бот корректно считает сообщения)          | ✅ READY  | Считает по реальному полю Author в истории                                 |
| 6  | MessageBox при ошибках чтения/записи в JSON-файл                      | ✅ READY  | Пользователь видит понятное предупреждение вместо консоли                  |
| 7  | Приветствие от бота при пустой истории чата                            | ✅ READY  | Показывается один раз при первом запуске или пустом файле истории          |
